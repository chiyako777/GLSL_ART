...
vec3 getNormal(vec3 p){
    ...
    return normalize(vec3(
        box_distanceFunc(p + vec3(  d, 0.0, 0.0)) - box_distanceFunc(p + vec3( -d, 0.0, 0.0)),
        box_distanceFunc(p + vec3(0.0,   d, 0.0)) - box_distanceFunc(p + vec3(0.0,  -d, 0.0)),
        box_distanceFunc(p + vec3(0.0, 0.0,   d)) - box_distanceFunc(p + vec3(0.0, 0.0,  -d))
        ));
}
...
void main(void){
    ...
    for(int i=0; i<64; i++){
        distance = box_distanceFunc(rPos);
        rLen += distance;
        rPos = cPos + ray * rLen;
    }
