...
void main(void){
    ...
    const float NL = 6.0;      //らせん係数(1にすると、単純な放射弾幕)
    const float NB = 4.0;      //弾幕放射回数
    ...
    const float FV = 0.4;       //初期弾速(1度目に放射される弾幕の速度係数)
    ...
    for(float l = 0.0; l < NL; l++){
        ...
        float rad = atan(p.x,p.y) + l / NL * RA;                                 //らせんを作っていく角度（NLの値によって少しずつ角度がずれていく）
