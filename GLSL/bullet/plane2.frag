...
void main(void){
    ...
    for(float k = -3.0; k < 3.0; k++){
        for(float j = 0.0; j < NB; j++){
            ...
            for(float i = 0.0; i < NL; i++){
                ...
                float velocity = mod((frameCount * 0.005),2)  + (j/NB * 0.2);
        
                //弾幕リセットタイミングでは描画しない（コメントしても変わらないけど、無駄処理しなくなる）
                //if( velocity == 0.0){ continue; }

                //弾幕色に塗りつぶすかが決定される
                float y = cos(angle) * velocity + cos(k/3.0);
                float x = sin(angle) * velocity + sin(k/3.0);
    ...
    gl_FragColor = vec4(vec3(col/3) , 1.0);
